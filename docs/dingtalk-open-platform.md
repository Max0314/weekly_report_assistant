...
2026-08-31 已使用正式应用和正式操作人逐表复核 AI 多维表只读链路：9 张同步表（含产品经理名单和新增使用反馈记录）的字段、记录接口均成功，`Notable.Base.Read.All` 权限正常。应用换版、操作人或 Base 协作者权限调整后，仍需重新执行“同步 AI 表”验证；验证失败时不得用假数据绕过。
...
- 审核人 `userId` 列表。
- 生产 HTTPS 域名；钉钉客户端必须能访问签名后的团队与个人周报链接。
