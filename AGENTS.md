...
- 周日 20:00 自动向正式目标发送当时的最新综合版；不要求确认、审核或先行预览，但仍必须校验最新版本、内容哈希、正式目标、公开链接和人员目录等推送依赖。
- 不使用假数据、mock 或硬编码绕过正式环境权限问题。

## 修改与验证

- 优先最小改动，中文文件保持 UTF-8。
- 当前阶段每次修改在测试、部署和健康检查全部通过后，必须将最新综合版周报按正式版样式推送到且仅推送到“推送测试”群；使用本次部署提交号作为测试推送幂等键，并核对发送结果。该测试推送不得改变周报审核/正式发送状态，也不得替代周日 20:00 的正式业务群自动发送。
...
- 推送前检查公开 HTTPS 地址、图片链接签名、人员目录缓存、最新综合版内容哈希、预览/正式群配置。
