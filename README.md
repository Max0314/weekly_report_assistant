...
- **团队与个人周报编辑**：详情页可从“外部打开”左侧进入团队周报编辑器，维护标题、六个总结区块、各分类摘要及本版每个事项的分类、状态、负责人、进展、计划和风险；个人周报允许本人维护个人总结、分类摘要及事项展示内容，审核人可代为编辑。旧预览链接会自动解析到同周期最新可编辑版本，多人依次从同一预览入口保存时会在数据库事务内基于最新版合并并保留其他人的个人覆盖。编辑只创建新的本地事实快照版本，不回写多维表；TB 官方项目状态保持只读。任一保存都会清除旧预览、图片和审核结果，要求重新走预览与审核流程。个人页不展示容易与 TB 项目进度混淆的事项完成率，仅保留分类分布和业务事实。
