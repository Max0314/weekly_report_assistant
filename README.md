...
- **固定周末编排与自动正式发送**：Asia/Shanghai 周六 09:00 生成最新综合版并仅投递“推送测试”群，周六 17:00 按正式样式单聊已配置的最终版接收人，周日 20:00 自动投递内容哈希有效的最新综合版，无需确认、审核或先行预览。
- **可追溯**：周报按周期和类型版本化；每次团队或个人保存都会创建新的综合版修订。人工审核记录仍绑定内容 SHA-256，但不再是周日正式发送门槛；消息在外部调用前原子占位并保存 `processQueryKey`，支持安全重试和群/个人撤回。
...
- **团队与个人周报编辑**：详情页可从“外部打开”左侧进入团队周报编辑器，维护标题、六个总结区块、各分类摘要及本版每个事项的分类、状态、负责人、进展、计划和风险；同周期同类型团队周报使用服务端独占编辑租约，其他人会看到当前编辑者提示，连续 30 分钟无实际操作后自动释放。编辑期间的 Web、机器人和周六调度生成请求会持久化排队，编辑结束后以最新来源重新生成并重放人工修改差异。个人周报不占团队锁，允许本人维护个人总结、分类摘要及事项展示内容，审核人可代为编辑；团队最终保存与延迟生成都会保留期间已保存的个人覆盖。编辑只创建新的本地事实快照版本，不回写多维表；TB 官方项目状态保持只读。任一保存都会生成新的最新综合版并清除旧图片、预览和审核结果；周日 20:00 仍自动发送当时已经保存的最新综合版。个人页不展示容易与 TB 项目进度混淆的事项完成率，仅保留分类分布和业务事实。
