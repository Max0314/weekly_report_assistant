...
- **团队与个人周报编辑**：团队综合版按“国内”和“海外 + 物联网”拆分，每个板块独立维护六章正文并保留同一周期、同一版本；默认打开国内，可通过顶部按钮切换。个人周报按七张源表呈现，单元格修改只生成当前版本的本地覆盖，不回写多维表；负责人、ProjectID 与 TB 官方状态只读并可恢复其他字段的源值。同周期同类型团队周报仍使用服务端独占编辑租约。任一保存都会生成新的最新综合版并清除旧预览和审核结果；周日 20:00 自动发送当时已保存的最新综合版。
- **链接式推送**：预览、测试群、周六最终版和周日正式发送仅发送卡片摘要与签名后的团队/个人周报入口，不自动生成或发送周报图片。手动渲染接口只用于内部排版检查。
...
| `PUBLIC_BASE_URL` / `PUBLIC_LINK_SECRET` | 钉钉可访问的 HTTPS 周报签名链接 |
